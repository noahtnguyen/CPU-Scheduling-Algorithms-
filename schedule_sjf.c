...
//          which schedules tasks in order of the length of the tasks' next CPU
//          burst.
// ----------------------------------------------------------------------------
#include "cpu.h"
#include "list.h"
...
void add(char *name, int priority, int burst) {
  ...
  // and insert based on burst time 
  else {
    struct node *cur = g_head;
    struct node *prev = NULL;
    // find the right place to insert
    while (cur != NULL && cur->task->burst <= task->burst) {
      prev = cur;
      cur = cur->next;
    }
